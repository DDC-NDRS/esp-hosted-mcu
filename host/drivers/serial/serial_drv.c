...
struct serial_drv_handle_t* serial_drv_open(char const* transport) {
    ...
    serial_drv_handle = (struct serial_drv_handle_t*)g_h.funcs->_h_calloc(1, sizeof(struct serial_drv_handle_t));
    if (!serial_drv_handle) {
...
uint8_t* serial_drv_read(struct serial_drv_handle_t* serial_drv_handle, uint32_t* out_nbyte) {
    ...
    char const* ep_name = RPC_EP_NAME_RSP;
    uint8_t*    buf     = NULL;
    uint32_t    buf_len = 0;
    ...
    *out_nbyte = 0;
    ...
    if ((!serial_ll_if_g) || (!serial_ll_if_g->fops) || (!serial_ll_if_g->fops->read)) {
    ...
    init_read_len = SIZE_OF_TYPE + SIZE_OF_LENGTH + strlen(ep_name) + SIZE_OF_TYPE + SIZE_OF_LENGTH;
...
int rpc_platform_init(void) {
    /* rpc semaphore */
    readSemaphore = g_h.funcs->_h_create_semaphore(H_MAX_SYNC_RPC_REQUESTS + H_MAX_ASYNC_RPC_REQUESTS);
