...
typedef enum { INIT, ACTIVE, DESTROY } serial_ll_state_e;
...
static uint8_t* serial_ll_read(serial_ll_handle_t const* serial_ll_hdl, uint16_t* rlen);
static int      serial_ll_write(serial_ll_handle_t const* serial_ll_hdl, uint8_t* wbuffer, uint16_t const wlen);
...
static int serial_ll_open(serial_ll_handle_t* serial_ll_hdl) {
    ...
    /* Queue - serial rx */
    serial_ll_hdl->queue = g_h.funcs->_h_create_queue(TO_SERIAL_INFT_QUEUE_SIZE, sizeof(interface_buffer_handle_t));
 ...
 */
static serial_ll_handle_t* get_serial_ll_handle(uint8_t const iface_num) {
    if ((iface_num < MAX_SERIAL_INTF) && (interface_handle_g[iface_num]) &&
...
static int serial_ll_write(serial_ll_handle_t const* serial_ll_hdl, uint8_t* wbuffer, uint16_t const wlen) {
    ...
    return esp_hosted_tx(serial_ll_hdl->if_type, serial_ll_hdl->if_num, wbuffer, wlen, H_BUFF_NO_ZEROCOPY,
                         H_DEFLT_FREE_FUNC);
}
...
#define SERIAL_ALLOC_REALLOC_RDATA()                                                                                   \
    ...
    if (g_h.funcs->_h_queue_item(serial_ll_hdl->queue, &new_buf_handle, HOSTED_BLOCK_MAX)) {
