    ...
    if (data[len] == PROTO_PSER_TLV_T_EPNAME) {
        ...
        if (val_len == strlen(ep_name)) {
            ...
            ESP_LOGE(TAG, "Endpoint length not matched, exp [For %s, %lu OR For %s, %lu], recvd %d\n", ep_name,
                     (long unsigned int)(strlen(ep_name)), ep_name2, (long unsigned int)(strlen(ep_name2)), val_len);
        }
    }
    else {
...
int transport_pserial_open(void) {
    int         ret       = SUCCESS;
    char const* transport = SERIAL_IF_FILE;
...
int transport_pserial_send(uint8_t* data, uint16_t data_length) {
     ...
     * --------------------------------------------------------------------------------------------
     */
    buf_len = SIZE_OF_TYPE + SIZE_OF_LENGTH + strlen(ep_name) + SIZE_OF_TYPE + SIZE_OF_LENGTH + data_length;
