...
static char const TAG[] = "H_SPI_HD_DRV";
    ...
    for (;;) {
        ...
        if (buf_handle.payload_len > MAX_SPI_HD_BUFFER_SIZE - sizeof(struct esp_payload_header)) {
            ESP_LOGE(TAG, "Pkt len [%u] > Max [%u]. Drop", buf_handle.payload_len,
                     MAX_SPI_HD_BUFFER_SIZE - sizeof(struct esp_payload_header));
            goto done;
        }
        ...
        buf_needed = (len + sizeof(struct esp_payload_header) + MAX_SPI_HD_BUFFER_SIZE - 1) / MAX_SPI_HD_BUFFER_SIZE;
...
static int is_valid_spi_hd_rx_packet(uint8_t* rxbuff_a, uint16_t* len_a, uint16_t* offset_a) {
    ...
    if (checksum != rx_checksum) {
        ESP_LOGE(TAG, "SPI_HD RX rx_chksum[%u] != checksum[%u]. Drop.", checksum, rx_checksum);
        return 0;
    }
#endif
...
void transport_init_internal(void) {
    ...
    for (prio_q_idx = 0; prio_q_idx < MAX_PRIORITY_QUEUES; prio_q_idx++) {
        ...
        from_slave_queue[prio_q_idx] =
            g_h.funcs->_h_create_queue(H_SPI_HD_RX_QUEUE_SIZE, sizeof(interface_buffer_handle_t));
        assert(from_slave_queue[prio_q_idx]);

        /* Queue - tx */
        to_slave_queue[prio_q_idx] =
            g_h.funcs->_h_create_queue(H_SPI_HD_TX_QUEUE_SIZE, sizeof(interface_buffer_handle_t));
    ...
    spi_hd_read_thread =
        g_h.funcs->_h_thread_create("spi_hd_read", DFLT_TASK_PRIO, DFLT_TASK_STACK_SIZE, spi_hd_read_task, NULL);

    spi_hd_process_rx_thread = g_h.funcs->_h_thread_create("spi_hd_process_rx", DFLT_TASK_PRIO, DFLT_TASK_STACK_SIZE,
                                                           spi_hd_process_rx_task, NULL);

    spi_hd_write_thread =
        g_h.funcs->_h_thread_create("spi_hd_write", DFLT_TASK_PRIO, DFLT_TASK_STACK_SIZE, spi_hd_write_task, NULL);
