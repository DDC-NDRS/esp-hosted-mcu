...
static char const TAG[] = "H_UART_DRV";
    ...
    while (true) {
        ...
        if (buf_handle.payload_len > MAX_UART_BUFFER_SIZE - sizeof(struct esp_payload_header)) {
            ESP_LOGE(TAG, "Pkt len [%u] > Max [%u]. Drop", buf_handle.payload_len,
                     MAX_UART_BUFFER_SIZE - sizeof(struct esp_payload_header));
            goto done;
        }
...
static int is_valid_uart_rx_packet(uint8_t* rxbuff_a, uint16_t* len_a, uint16_t* offset_a) {
    ...
    if (checksum != rx_checksum) {
        ESP_LOGE(TAG, "UART RX rx_chksum[%u] != checksum[%u]. Drop.", checksum, rx_checksum);
        return 0;
    }
#endif
...
void transport_init_internal(void) {
    ...
    for (prio_q_idx = 0; prio_q_idx < MAX_PRIORITY_QUEUES; prio_q_idx++) {
        ...
        from_slave_queue[prio_q_idx] =
            g_h.funcs->_h_create_queue(H_UART_RX_QUEUE_SIZE, sizeof(interface_buffer_handle_t));
        assert(from_slave_queue[prio_q_idx]);

        /* Queue - tx */
        to_slave_queue[prio_q_idx] =
            g_h.funcs->_h_create_queue(H_UART_TX_QUEUE_SIZE, sizeof(interface_buffer_handle_t));
    ...
    h_uart_process_rx_task_info = g_h.funcs->_h_thread_create("uart_process_rx", DFLT_TASK_PRIO, DFLT_TASK_STACK_SIZE,
                                                              h_uart_process_rx_task, NULL);

    h_uart_read_task_info =
        g_h.funcs->_h_thread_create("uart_rx", DFLT_TASK_PRIO, DFLT_TASK_STACK_SIZE, h_uart_read_task, NULL);

    h_uart_write_task_info =
        g_h.funcs->_h_thread_create("uart_tx", DFLT_TASK_PRIO, DFLT_TASK_STACK_SIZE, h_uart_write_task, NULL);
}
