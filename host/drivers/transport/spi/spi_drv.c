...
void transport_init_internal(void) {
    ...
    spi_transaction_thread =
        g_h.funcs->_h_thread_create("spi_trans", DFLT_TASK_PRIO, DFLT_TASK_STACK_SIZE, spi_transaction_task, NULL);
    assert(spi_transaction_thread);

    /* Task - RX processing */
    spi_rx_thread =
        g_h.funcs->_h_thread_create("spi_rx", DFLT_TASK_PRIO, DFLT_TASK_STACK_SIZE, spi_process_rx_task, NULL);
...
static int process_spi_rx_buf(uint8_t* rxbuff) {
    ...
    if ((len > MAX_PAYLOAD_SIZE) || (offset != sizeof(struct esp_payload_header))) {
...
static int check_and_execute_spi_transaction(void) {
    ...
    gpio_handshake = g_h.funcs->_h_read_gpio(H_GPIO_HANDSHAKE_Port, H_GPIO_HANDSHAKE_Pin);

    /* data ready line SET -> slave wants to send something */
    gpio_rx_data_ready = g_h.funcs->_h_read_gpio(H_GPIO_DATA_READY_Port, H_GPIO_DATA_READY_Pin);
    ...
    if (gpio_handshake == H_HS_VAL_ACTIVE) {
        ...
        if ((gpio_rx_data_ready == H_DR_VAL_ACTIVE) || (is_valid_tx_buf) || schedule_dummy_tx || schedule_dummy_rx) {
...
#if ESP_PKT_STATS
            struct esp_payload_header* payload_header = (struct esp_payload_header*)txbuff;
...
int esp_hosted_tx(uint8_t iface_type, uint8_t iface_num, uint8_t* wbuffer, uint16_t wlen, uint8_t buff_zcopy,
    ...
    if (!wbuffer || !wlen || (wlen > MAX_PAYLOAD_SIZE) || !transport_up) {
        ESP_LOGE(TAG, "write fail: trans_ready[%u] buff(%p) 0? OR (0<len(%u)<=max_poss_len(%u))?", transport_up,
...
static void spi_transaction_task(void const* pvParameters) {
    ...
    g_h.funcs->_h_config_gpio_as_interrupt(H_GPIO_HANDSHAKE_Port, H_GPIO_HANDSHAKE_Pin, H_HS_INTR_EDGE,
                                           gpio_hs_isr_handler);

    g_h.funcs->_h_config_gpio_as_interrupt(H_GPIO_DATA_READY_Port, H_GPIO_DATA_READY_Pin, H_DR_INTR_EDGE,
                                           gpio_dr_isr_handler);
    ...
    for (;;) {
        ...
        if ((!is_transport_rx_ready()) || (!spi_trans_ready_sem)) {
