...
static int process_rpc_tx_msg(ctrl_cmd_t* app_req) {
    ...
    if (app_req->rpc_rsp_cb) {
        async_timer_hdl =
            g_h.funcs->_h_timer_start(app_req->rsp_timeout_sec, RPC__TIMER_ONESHOT, rpc_async_timeout_handler, app_req);
        if (!async_timer_hdl) {
...
static int process_rpc_rx_msg(Rpc* proto_msg, rpc_rx_ind_t rpc_rx_func) {
    ...
    if (proto_msg->msg_type == RPC_TYPE__Event) {
        ...
        if (CALLBACK_AVAILABLE == is_event_callback_registered(proto_msg->msg_id)) {
        ...
        if (CALLBACK_AVAILABLE == is_async_resp_callback_available(app_resp)) {
...
static int spawn_rpc_threads(void) {
    ...
    rpc_rx_thread_hdl = g_h.funcs->_h_thread_create("rpc_rx", RPC_TASK_PRIO, RPC_TASK_STACK_SIZE, rpc_rx_thread, NULL);
    rpc_tx_thread_hdl = g_h.funcs->_h_thread_create("rpc_tx", RPC_TASK_PRIO, RPC_TASK_STACK_SIZE, rpc_tx_thread, NULL);
...
static int wait_for_sync_response(ctrl_cmd_t* app_req) {
    ...
    ESP_LOGV(TAG, "Wait for sync resp for Req[0x%x] with timer of %u sec", app_req->msg_id, timeout_sec);
    for (i = 0; i < MAX_SYNC_RPC_TRANSACTIONS; i++) {
...
int rpc_core_init(void) {
    ...
    rpc_rx_q = g_h.funcs->_h_create_queue(RPC_RX_QUEUE_SIZE, sizeof(esp_queue_elem_t));

    rpc_tx_q = g_h.funcs->_h_create_queue(RPC_TX_QUEUE_SIZE, sizeof(void*));
