 ...
 */
int ipv4_addr_aton(char const* cp, uint32_t* ip_uint32) {
    u_long val, base, n;
    char   c;
 ...
 */
stm_ret_t convert_mac_to_bytes(uint8_t* out, char const* s) {
    int mac[MAC_LEN] = {0};
    int num_bytes    = 0;
    ...
    num_bytes = sscanf(s, "%2x:%2x:%2x:%2x:%2x:%2x", &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]);

    if ((num_bytes < MAC_LEN) || (mac[0] > 0xFF) || (mac[1] > 0xFF) || (mac[2] > 0xFF) || (mac[3] > 0xFF) ||
 ...
 * @retval STM_OK/STM_FAIL
 */
stm_ret_t get_ipaddr_from_str(char const* ip_s, uint32_t* ip_x) {
    uint32_t ip_nw = 0;
