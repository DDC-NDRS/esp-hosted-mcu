...
#if USE_FLOATING_POINT
    ESP_LOGI(TAG, "%lu-%lu sec Tx:%.2f Rx:%.2f kbps\n\r", raw_tp_timer_count, raw_tp_timer_count + TEST_RAW_TP__TIMEOUT,
             actual_bandwidth_tx / div, actual_bandwidth_rx / div);
#else
    ESP_LOGI(TAG, "%lu-%lu sec Tx:%lu Rx:%lu Kbps", raw_tp_timer_count, raw_tp_timer_count + TEST_RAW_TP__TIMEOUT,
             (unsigned long)actual_bandwidth_tx / div, (unsigned long)actual_bandwidth_rx / div);
#endif
...
static void process_raw_tp_flags(uint8_t cap) {
    ...
    if (test_raw_tp) {
        hosted_timer_handler =
            g_h.funcs->_h_timer_start(TEST_RAW_TP__TIMEOUT, RPC__TIMER_PERIODIC, raw_tp_timer_func, NULL);
        ...
        if ((cap & ESP_TEST_RAW_TP__HOST_TO_ESP) || (cap & ESP_TEST_RAW_TP__BIDIRECTIONAL)) {
            raw_tp_tx_task_id = g_h.funcs->_h_thread_create("raw_tp_tx", DFLT_TASK_PRIO, RAW_TP_TX_TASK_STACK_SIZE,
                                                            raw_tp_tx_task, NULL);
...
void stats_timer_func(void* arg) {
    ESP_LOGI(TAG,
             "slave: sta_rx_in: %lu sta_rx_out: %lu sta_tx_in [pass: %lu drop: %lu] sta_tx_out: %lu, throttling %u",
...
void create_debugging_tasks(void) {
    ...
    if (ESP_PKT_STATS_REPORT_INTERVAL) {
        ...
        pkt_stats_thread =
            g_h.funcs->_h_timer_start(ESP_PKT_STATS_REPORT_INTERVAL, RPC__TIMER_PERIODIC, stats_timer_func, NULL);
