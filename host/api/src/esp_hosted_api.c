...
esp_remote_channel_t esp_hosted_add_channel(esp_remote_channel_config_t config,
                                            esp_remote_channel_tx_fn_t* tx, esp_remote_channel_rx_fn_t const rx) {
...
esp_err_t esp_wifi_remote_init(wifi_init_config_t const* arg) {
...
esp_err_t esp_wifi_remote_set_mac(wifi_interface_t mode, uint8_t const mac[6]) {
    return rpc_wifi_set_mac(mode, mac);
}

esp_err_t esp_wifi_remote_scan_start(wifi_scan_config_t const* config, bool block) {
...
esp_err_t esp_wifi_remote_set_country_code(char const* country, bool ieee80211d_enabled) {
...
esp_err_t esp_wifi_remote_set_country(wifi_country_t const* country) {
...
esp_err_t esp_wifi_remote_ap_get_sta_aid(uint8_t const mac[6], uint16_t* aid) {
