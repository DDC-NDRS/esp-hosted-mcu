...
typedef struct {
    /* Memory */
    /* 1 */ void* (*_h_memcpy)(void* dest, void const* src, uint32_t size);
    ...
    /* Thread */
    /* 11 */ void* (*_h_thread_create)(const char* tname, uint32_t tprio, uint32_t tstack_size,
    ...
    /* 38 */ int (*_h_config_gpio_as_interrupt)(void* gpio_port, uint32_t gpio_num, uint32_t intr_type,
...
#endif
    /* 43 */ int (*_h_event_wifi_post)(int32_t event_id, void* event_data, size_t event_data_size,
                                       uint32_t ticks_to_wait);
    /* 44 */ void (*_h_printf)(int level, char const* tag, char const* format, ...);
