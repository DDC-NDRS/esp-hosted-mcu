...
#include "esp_log.h"
static char const TAG[] = "spi_hd_wrapper";
...
#define SPI_HD_FAIL_IF_NULL(x)                                                                                         \
    do {                                                                                                               \
        if (!x)                                                                                                        \
            return ESP_FAIL;                                                                                           \
    } while (0);

#define SPI_HD_LOCK(x)                                                                                                 \
    do {                                                                                                               \
        if (x)                                                                                                         \
            g_h.funcs->_h_lock_mutex(spi_hd_bus_lock, portMAX_DELAY);                                                  \
    } while (0);

#define SPI_HD_UNLOCK(x)                                                                                               \
    do {                                                                                                               \
        if (x)                                                                                                         \
            g_h.funcs->_h_unlock_mutex(spi_hd_bus_lock);                                                               \
    } while (0);
...
static esp_err_t spi_hd_write_reg(uint8_t const* data, int addr, int len, uint32_t flags) {
    spi_transaction_ext_t t = {
        .base =
            {
...
static esp_err_t spi_hd_wrdma_seg(uint8_t const* data, int seg_len, uint32_t flags) {
#if USE_DMA_ALIGNED_BUF
...
static esp_err_t spi_hd_wrdma(uint8_t const* data, int len, int seg_len, uint32_t flags) {
...
int hosted_spi_hd_set_data_lines(uint32_t data_lines) {
    ...
    else if (data_lines == H_SPI_HD_CONFIG_4_DATA_LINES) {
