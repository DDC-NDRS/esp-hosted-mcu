...
void* hosted_memcpy(void* dest, void const* src, uint32_t size) {
    if (size && (!dest || !src)) {
        if (!dest) {
...
void* hosted_thread_create(char* tname, uint32_t tprio, uint32_t tstack_size, void (*start_routine)(void const*),
                           void* sr_arg) {
    ...
    thread_handle_t* thread_handle = (thread_handle_t*)hosted_malloc(sizeof(thread_handle_t));
    if (!thread_handle) {
...
void* hosted_create_queue(uint32_t qnum_elem, uint32_t qitem_size) {
    ...
    q_id = (queue_handle_t*)hosted_malloc(sizeof(queue_handle_t));
    if (!q_id) {
...
void* hosted_create_mutex(void) {
    ...
    mut_id = (mutex_handle_t*)hosted_malloc(sizeof(mutex_handle_t));
...
void* hosted_create_semaphore(int maxCount) {
    ...
    sem_id = (semaphore_handle_t*)hosted_malloc(sizeof(semaphore_handle_t));
    if (!sem_id) {
...
static void* hosted_create_spinlock(void) {
    ...
    spin_id = (spinlock_handle_t*)hosted_malloc(sizeof(spinlock_handle_t));
...
void* hosted_timer_start(int duration, int type, void (*timeout_handler)(void*), void* arg) {
    ...
    esp_timer_create_args_t const timerNew_args = {
...
void hosted_log_write(int level, char const* tag, char const* format, ...) {
    va_list list;
