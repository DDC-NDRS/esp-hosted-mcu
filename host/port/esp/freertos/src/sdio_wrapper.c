...
#define SDIO_LOCK(x)                                                                                                   \
    do {                                                                                                               \
        if (x)                                                                                                         \
            g_h.funcs->_h_lock_mutex(sdio_bus_lock, portMAX_DELAY);                                                    \
    } while (0);

#define SDIO_UNLOCK(x)                                                                                                 \
    do {                                                                                                               \
        if (x)                                                                                                         \
            g_h.funcs->_h_unlock_mutex(sdio_bus_lock);                                                                 \
    } while (0);
...
static esp_err_t hosted_sdio_set_blocksize(sdmmc_card_t* card, uint8_t fn, uint16_t value) {
    ...
    uint8_t const* bs_u8      = (uint8_t const*)&value;
    uint16_t       bs_read    = 0;
...
static esp_err_t sdio_read_fromio(sdmmc_card_t* card, uint32_t function, uint32_t addr, uint8_t* data, uint16_t size) {
...
static esp_err_t sdio_write_toio(sdmmc_card_t* card, uint32_t function, uint32_t addr, uint8_t* data, uint16_t size) {
    uint16_t  remainder = size;
    uint16_t  blocks;
    esp_err_t res;
    uint8_t*  ptr = data;
...
int hosted_sdio_card_init(void* ctx) {
    ...
    ESP_LOGI(TAG, "SDIO master: Slot %d, Data-Lines: %d-bit Freq(KHz)[%u KHz]", config.slot,
             sdio_config->bus_width == 4 ? 4 : 1, config.max_freq_khz);
    if (sdio_config->bus_width == 4) {
        ESP_LOGI(TAG, "GPIOs: CLK[%u] CMD[%u] D0[%u] D1[%u] D2[%u] D3[%u] Slave_Reset[%u]", sdio_config->pin_clk.pin,
                 sdio_config->pin_cmd.pin, sdio_config->pin_d0.pin, sdio_config->pin_d1.pin, sdio_config->pin_d2.pin,
                 sdio_config->pin_d3.pin, H_GPIO_PIN_RESET_Pin);
    }
    else {
        ESP_LOGI(TAG, "GPIOs: CLK[%u] CMD[%u] D0[%u] D1[%u] Slave_Reset[%u]", sdio_config->pin_clk.pin,
                 sdio_config->pin_cmd.pin, sdio_config->pin_d0.pin, sdio_config->pin_d1.pin, H_GPIO_PIN_RESET_Pin);
    }
    ESP_LOGI(TAG, "Queues: Tx[%u] Rx[%u] SDIO-Rx-Mode[%u]", H_SDIO_TX_Q, H_SDIO_RX_Q, H_SDIO_HOST_RX_MODE);
