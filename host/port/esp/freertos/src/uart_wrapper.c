...
#include "esp_log.h"
static char const TAG[] = "uart_wrapper";
...
#define UART_FAIL_IF_NULL_CTX(x)                                                                                       \
    do {                                                                                                               \
        if (!x)                                                                                                        \
            return ESP_FAIL;                                                                                           \
    } while (0);
...
int hosted_uart_write(void* ctx, uint8_t* data, uint16_t size) {
    ...
    return uart_write_bytes(pctx->uart_port, (char const*)data, size);
}
...
void* hosted_uart_init(void) {
    ...
    ESP_ERROR_CHECK(uart_driver_install(H_UART_PORT, MAX_UART_BUFFER_SIZE, MAX_UART_BUFFER_SIZE, 0, NULL, 0));
    ESP_ERROR_CHECK(uart_param_config(H_UART_PORT, &uart_config));
    ESP_ERROR_CHECK(uart_set_pin(H_UART_PORT, H_UART_TX_PIN, H_UART_RX_PIN, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));

    ESP_LOGI(TAG, "UART GPIOs: Tx: %" PRIu16 ", Rx: %" PRIu16 ", Baud Rate %i", H_UART_TX_PIN, H_UART_RX_PIN,
             H_UART_BAUD_RATE);
