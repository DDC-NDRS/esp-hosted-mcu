...
static char const TAG[] = "protocomm_pserial";
...
static esp_err_t rpc_req_handler(protocomm_t* pc, uint8_t* in, size_t in_len) {
    uint8_t* buf       = in;
...
static esp_err_t rpc_evt_handler(protocomm_t* pc, uint8_t* in, size_t in_len, int msg_id) {
    int ret = 0;
    ...
    ret = protocomm_req_handle(pc, epname, msg_id, in, in_len, &out, (ssize_t*)&outlen);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Error in handling protocomm request %d", ret);
        return ESP_FAIL;
    }
...
static esp_err_t rpc_add_ep(char const* ep_name, protocomm_req_handler_t req_handler, void* priv_data) {
    ESP_LOGD(TAG, "Adding endpoint for rpc");
    return ESP_OK;
}

static esp_err_t rpc_remove_ep(char const* ep_name) {
...
static void pserial_task(void* params) {
    ...
    while (xQueueReceive(pserial_cfg->req_queue, &arg, portMAX_DELAY) == pdTRUE) {
        if ((arg.msg_id > RPC_ID__Event_Base) && (arg.msg_id < RPC_ID__Event_Max)) {
            /* Events */
...
esp_err_t protocomm_pserial_start(protocomm_t* pc, pserial_xmit xmit, pserial_recv recv) {
    ...
    xTaskCreate(pserial_task, "pserial_task", 1024 * 5, (void*)pc, CONFIG_ESP_DEFAULT_TASK_PRIO, NULL);
