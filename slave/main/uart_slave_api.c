...
static char const TAG[] = "UART_DRIVER";
...
static inline void h_uart_mempool_create() {
    buf_mp_tx_g = hosted_mempool_create(NULL, 0, HOSTED_UART_TX_QUEUE_SIZE, BUFFER_SIZE);
    buf_mp_rx_g = hosted_mempool_create(NULL, 0, HOSTED_UART_RX_QUEUE_SIZE, BUFFER_SIZE);
...
static void uart_rx_task(void* pvParameters) {
    ...
    while (1) {
        // get the header
        bytes_read = uart_read_bytes(HOSTED_UART, uart_scratch_buf, sizeof(struct esp_payload_header), portMAX_DELAY);
        ...
        // get the data
        bytes_read = uart_read_bytes(HOSTED_UART, &uart_scratch_buf[offset], len, portMAX_DELAY);
...
static int32_t h_uart_write(interface_handle_t* handle, interface_buffer_handle_t* buf_handle) {
    ...
    tx_len = uart_write_bytes(HOSTED_UART, (char const*)sendbuf, total_len);
...
static interface_handle_t* h_uart_init(void) {
    ...
    ESP_ERROR_CHECK(uart_driver_install(HOSTED_UART, BUFFER_SIZE, BUFFER_SIZE, 0, NULL, 0));
    ESP_ERROR_CHECK(uart_param_config(HOSTED_UART, &uart_config));
    ESP_ERROR_CHECK(
        uart_set_pin(HOSTED_UART, HOSTED_UART_GPIO_TX, HOSTED_UART_GPIO_RX, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
    ESP_LOGI(TAG, "UART GPIOs: Tx: %" PRIu16 ", Rx: %" PRIu16 ", Baud Rate %i", HOSTED_UART_GPIO_TX,
             HOSTED_UART_GPIO_RX, HOSTED_UART_BAUD_RATE);
    ESP_LOGI(TAG, "Hosted UART Queue Sizes: Tx: %" PRIu16 ", Rx: %" PRIu16, HOSTED_UART_TX_QUEUE_SIZE,
             HOSTED_UART_RX_QUEUE_SIZE);
    ...
    // start up tasks
    assert(xTaskCreate(uart_rx_task, "uart_rx_task", CONFIG_ESP_DEFAULT_TASK_STACK_SIZE, NULL,
