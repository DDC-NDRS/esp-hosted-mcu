...
#include "esp_log.h"
static char const TAG[] = "SPI_HD_DRIVER";
...
#define MAKE_SPI_HD_DMA_ALIGNED(VAL) (VAL += SPI_HD_DMA_ALIGNMENT_BYTES - ((VAL) & SPI_HD_DMA_ALIGNMENT_MASK))
...
#if H_DATAREADY_ACTIVE_HIGH
#define set_dataready_gpio()                                                                                           \
    {                                                                                                                  \
        data_ready_gpio_active = true;                                                                                 \
        gpio_set_level(GPIO_DATA_READY, 1);                                                                            \
    }
#define reset_dataready_gpio()                                                                                         \
    {                                                                                                                  \
        gpio_set_level(GPIO_DATA_READY, 0);                                                                            \
        data_ready_gpio_active = false;                                                                                \
    }
#else
#define set_dataready_gpio()                                                                                           \
    {                                                                                                                  \
...
static inline void spi_hd_mempool_create() {
    buf_mp_tx_g = hosted_mempool_create(NULL, 0, TX_MEMPOOL_NUM_BLOCKS, SPI_HD_BUFFER_SIZE);
    trans_tx_g  = hosted_mempool_create(NULL, 0, TX_MEMPOOL_NUM_BLOCKS, sizeof(spi_slave_hd_data_t));
    buf_mp_rx_g = hosted_mempool_create(NULL, 0, RX_MEMPOOL_NUM_BLOCKS, SPI_HD_BUFFER_SIZE);
    trans_rx_g  = hosted_mempool_create(NULL, 0, RX_MEMPOOL_NUM_BLOCKS, sizeof(spi_slave_hd_data_t));
...
static bool cb_rx_ready(void* arg, spi_slave_hd_event_t* event, BaseType_t* awoken) {
    ...
    // update count
    rx_ready_buf_num++;
    spi_slave_hd_write_buffer(SPI_HOST, SPI_HD_REG_RX_BUF_LEN, (uint8_t*)&rx_ready_buf_num, sizeof(rx_ready_buf_num));
...
static bool cb_tx_ready(void* arg, spi_slave_hd_event_t* event, BaseType_t* awoken) {
    ...
    tx_ready_buf_size = ((tx_ready_buf_size & SPI_HD_TX_BUF_LEN_MASK) + event->trans->len) & SPI_HD_TX_BUF_LEN_MASK;

    // restore the int mask;
    tx_ready_buf_size |= int_mask;

    spi_slave_hd_write_buffer(SPI_HOST, SPI_HD_REG_TX_BUF_LEN, (uint8_t*)&tx_ready_buf_size, sizeof(tx_ready_buf_size));
...
static void spi_hd_read_done(void* handle) {
    ...
    res                        = spi_slave_hd_queue_trans(SPI_HOST, SPI_SLAVE_CHAN_RX, trans, portMAX_DELAY);
    if (res) {
...
static void spi_hd_rx_task(void* pvParameters) {
    ...
    while (1) {
        ...
        res = spi_slave_hd_get_trans_res(SPI_HOST, SPI_SLAVE_CHAN_RX, &ret_trans, portMAX_DELAY);
        if (ret) {
        ...
        if (checksum != rx_checksum) {
            ESP_LOGE(TAG, "%s: cal_chksum[%u] != exp_chksum[%u], drop len[%u] offset[%u]", __func__, checksum,
                     rx_checksum, len, offset);
            // return the transaction back to the rx queue
            spi_hd_read_done(ret_trans);
            continue;
        }
...
static void spi_hd_tx_done_task(void* pvParameters) {
    ...
    while (1) {
        err = spi_slave_hd_get_trans_res(SPI_HOST, SPI_SLAVE_CHAN_TX, &ret_trans, portMAX_DELAY);
        if (err == ESP_OK) {
...
static interface_handle_t* esp_spi_hd_init(void) {
    ...
    gpio_config_t io_data_ready_conf = {
        .intr_type = GPIO_INTR_DISABLE, .mode = GPIO_MODE_OUTPUT, .pin_bit_mask = GPIO_MASK_DATA_READY};
    ...
    ESP_LOGI(TAG, "SPI HD Host:%" PRIu16 " mode: %" PRIu16 ", Freq:ConfigAtHost", SPI_HOST, slave_hd_cfg.mode);
#if (NUM_DATA_BITS == 4)
    ESP_LOGI(TAG,
             "SPI HD GPIOs: Dat0: %" PRIu16 ", Dat1: %" PRIu16 ", Dat2: %" PRIu16 ", Dat3: %" PRIu16 ", CS: %" PRIu16
             ", CLK: %" PRIu16 ", Data Ready: %" PRIu16,
             GPIO_D0, GPIO_D1, GPIO_D2, GPIO_D3, GPIO_CS, GPIO_SCLK, GPIO_DATA_READY);
#else
    ESP_LOGI(TAG,
             "SPI HD GPIOs: Dat0: %" PRIu16 ", Dat1: %" PRIu16 ", CS: %" PRIu16 ", CLK: %" PRIu16
             ", Data Ready: %" PRIu16,
    ...
    spi_slave_hd_write_buffer(SPI_HOST, SPI_HD_REG_MAX_TX_BUF_LEN, (uint8_t*)&value, sizeof(value));

    value = SPI_HD_BUFFER_SIZE;
    spi_slave_hd_write_buffer(SPI_HOST, SPI_HD_REG_MAX_RX_BUF_LEN, (uint8_t*)&value, sizeof(value));

    // SPI HD SLave is ready
    value = SPI_HD_STATE_SLAVE_READY;
    spi_slave_hd_write_buffer(SPI_HOST, SPI_HD_REG_SLAVE_READY, (uint8_t*)&value, sizeof(value));
    ...
    assert(xTaskCreate(spi_hd_rx_task, "spi_hd_rx_task", CONFIG_ESP_DEFAULT_TASK_STACK_SIZE, NULL,
                       CONFIG_ESP_DEFAULT_TASK_PRIO, NULL) == pdTRUE);

    // task to clean up after doing tx
    assert(xTaskCreate(spi_hd_tx_done_task, "spi_hd_tx_done_task", CONFIG_ESP_DEFAULT_TASK_STACK_SIZE, NULL,
                       CONFIG_ESP_DEFAULT_TASK_PRIO, NULL) == pdTRUE);

    assert(xTaskCreate(flow_ctrl_task, "flow_ctrl_task", CONFIG_ESP_DEFAULT_TASK_STACK_SIZE, NULL,
                       CONFIG_ESP_DEFAULT_TASK_PRIO, NULL) == pdTRUE);
...
static int32_t esp_spi_hd_write(interface_handle_t* handle, interface_buffer_handle_t* buf_handle) {
    ...
    ret = spi_slave_hd_queue_trans(SPI_HOST, SPI_SLAVE_CHAN_TX, tx_trans, portMAX_DELAY);
    if (ret != ESP_OK) {
