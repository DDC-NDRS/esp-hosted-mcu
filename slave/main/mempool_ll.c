...
STAILQ_HEAD(, os_mempool) g_os_hosted_mempool_list = STAILQ_HEAD_INITIALIZER(g_os_hosted_mempool_list);
...
static void os_mempool_poison(void* start, int sz) {
    ...
    for (i = sizeof(struct os_memblock); i < sz; i = i + sizeof(os_mem_poison)) {
...
static void os_mempool_poison_check(void* start, int sz) {
    ...
    for (i = sizeof(struct os_memblock); i < sz; i = i + sizeof(os_mem_poison)) {
        assert(!memcmp(p + i, &os_mem_poison, min(sizeof(os_mem_poison), sz - i)));
    }
}
#else
...
os_error_t os_mempool_init(struct os_mempool* mp, uint16_t blocks, uint32_t block_size, void* membuf,
                           char const* name) {
    int                 true_block_size;
    uint8_t*            block_addr;
...
os_error_t os_mempool_ext_init(struct os_mempool_ext* mpe, uint16_t blocks, uint32_t block_size, void* membuf,
                               char const* name) {
    int rc;
...
int os_memblock_from(const struct os_mempool* mp, void const* block_addr) {
    uintptr_t true_block_size;
    uintptr_t baddr_ptr;
    uintptr_t end;

    _Static_assert(sizeof block_addr == sizeof baddr_ptr, "Pointer to void must be native word size.");
